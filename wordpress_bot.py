...
def generate_blog_content(news_text):
    ...
    **3. 숫자는 현실적이고 검증 가능한 비유로 설명** ⚠️ 매우 중요!
    
    ✅ **올바른 예시** (실제로 존재하는 가격):
    - "100만원" → "스타벅스 커피 약 200잔", "중급 스마트폰 1대", "1박 2일 제주도 여행"
    - "1,000만원" → "준중형 중고차", "유럽 한 달 여행", "전세 보증금 일부"
    - "1억원" → "지방 소형 아파트", "서울 전세금 일부"
    - "1조원" → "대기업 연간 매출", "중소 도시 1년 예산"
    - "250조원" → "대한민국 1년 국가 예산의 절반 정도"
    ...
    **계산 예시**:
    - "3% 상승" → "100만원이 103만원으로 증가"
    - "5.25% 금리" → "1억원 대출 시 연간 이자 525만원"
    - "1,400원 환율" → "1달러를 바꾸려면 1,400원 필요"
    - "250조 투자" → "국민 1인당 약 500만원씩 투자한 것과 같은 규모"
    
    **비유 작성 원칙**:
    1. 2026년 기준 실제 가격 사용
    2. 누구나 알 수 있는 일상적 물건/경험
    3. 검증 가능한 사실만 사용
    4. 과장하지 않기
    ...
    **⚠️ 중요: 아래 모든 섹션을 반드시 포함하세요. 뉴스가 적어도 각 섹션을 상세히 작성하여 최소 5,000자를 채우세요!**
    ...
    8. **현실적 비유**: 실제로 존재하는 가격과 검증 가능한 사실만 사용 (100만원짜리 자동차 같은 터무니없는 비유 절대 금지)
